...
export default function App() {
  ...
  return (
    <div className="App">
      ...
      <ReactDadataBox
        token={testToken}
        onChange={(suggestion) => setSample1(suggestion)}
      />
      <br />
      ...
      {/* Sample of custom service typings as 'party'*/}
      <ReactDadataBox<"party">
        token={testToken}
        type="party"
        onChange={(suggestion: PartyResponseType) => setSample2(suggestion)}
        customActions={(suggestions: PartyResponseType[]) =>
          !suggestions.length && (
      ...
      <ReactDadataBox
        ...
        customActions={() =>
          ["действие1", "действие1"].map((action) => (
